...
export default function NewListingPage() {
  ...
  const [kind, setKind] = useState<'SALE' | 'SHOWCASE'>('SHOWCASE');
  ...
  const handleSubmit = async () => {
    ...
    const parsed = createListingSchema.safeParse({
      kind, title, category, description, techStack, previewUrl, thumbnailUrl,
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        <div className="page-container py-4 max-w-lg mx-auto space-y-5">
          ...
          <div className="flex gap-2">
            {(['SHOWCASE', 'SALE'] as const).map((k) => (
              <button key={k} onClick={() => setKind(k)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border transition-colors ${
                  kind === k ? 'bg-accent-500 text-white border-accent-500' : 'bg-surface text-txt-secondary border-border-subtle'
                }`}>
                {k === 'SALE' ? '🛒 For Sale' : '🚀 Showcase'}
              </button>
            ))}
          </div>
