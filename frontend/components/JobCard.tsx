...
const STATUS_CONFIG: Record<string, { label: string; badge: string; overlay?: string }> = {
  OPEN: { label: 'Available', badge: 'bg-success-50 text-success-600' },
  NEGOTIATING: { label: 'Negotiating', badge: 'bg-warning-50 text-warning-600' },
  IN_PROGRESS: { label: 'In Progress', badge: 'bg-accent-50 text-accent-700' },
  COMPLETED: { label: 'Completed', badge: 'bg-inset text-txt-tertiary' },
  CANCELLED: { label: 'Cancelled', badge: 'bg-danger-50 text-danger-600' },
};
...
export function JobCard({ job }: { job: FeedJob }) {
  ...
  async function handleOpen() {
    ...
    if (isMyNegotiation && myNegotiationId) {
      router.push(`/negotiate/${myNegotiationId}`);
      return;
    }
    setLocking(true);
    try {
      ...
      if (res && res.ok) {
        ...
        router.push(`/negotiate/${res.negotiation.id}`);
      } else {
  ...
  return (
    <div
      className={`group relative flex flex-col overflow-hidden rounded-xl border bg-surface transition-all duration-200 ${
        clickable
          ? 'cursor-pointer border-border-subtle shadow-soft hover:shadow-medium hover:border-accent-200'
          : isMyNegotiation
    ...
    >
      <div className="flex flex-1 flex-col p-4">
        <div className="mb-2 flex items-start justify-between gap-2">
          ...
          <span className="shrink-0 rounded-md bg-accent-50 px-2 py-0.5 text-xs font-semibold text-accent-700">
